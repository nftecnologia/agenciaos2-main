...
const JobProgress = ({ jobStatus, title }: { jobStatus: JobStatus | null, title: string }) => {
  ...
  const getStatusText = () => {
    switch (jobStatus.step) {
      case 'description':
        return 'Gerando descrição com IA...'
      case 'content':
        return 'Criando conteúdo do ebook...'
      case 'pdf':
        return 'Gerando PDF final...'
      default:
        return 'Processando...'
    }
  }
  ...
  return (
    <Card className="p-6 mb-6">
      ...
      <p className="text-xs text-gray-500">{jobStatus.progress}% concluído</p>
      
      {jobStatus.status === 'failed' && jobStatus.failedReason && (
        <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
          <p className="text-sm text-red-700">Erro: {jobStatus.failedReason}</p>
        </div>
      )}
    </Card>
  )
...
export function EbookWizardQueue({ onComplete }: EbookWizardProps) {
  ...
  const startPolling = (jobId: string) => {
    ...
    const pollJobStatus = async () => {
      try {
        const response = await fetch(`/api/ebook/queue/status/${jobId}`)
        if (response.ok) {
    ...
    pollJobStatus() // Primeira verificação imediata
  }
  ...
  const handleJobCompleted = (completedJob: JobStatus) => {
    setIsLoading(false)
    
    switch (completedJob.step) {
      case 'description':
        if (completedJob.result && typeof completedJob.result === 'object' && 'description' in completedJob.result) {
          setGeneratedDescription(completedJob.result.description as Record<string, unknown>)
          setEditedDescription((completedJob.result.description as { description: string }).description)
          setCurrentStep(2)
        }
        break
      case 'content':
        if (completedJob.result && typeof completedJob.result === 'object' && 'content' in completedJob.result) {
          setGeneratedContent(completedJob.result.content as Record<string, unknown>)
          setCurrentStep(4)
        }
        break
      case 'pdf':
        if (completedJob.result && typeof completedJob.result === 'object' && 'pdfUrl' in completedJob.result) {
          setPdfUrl(completedJob.result.pdfUrl as string)
          if (ebook) {
            onComplete({ ...ebook, pdfUrl: completedJob.result.pdfUrl as string })
          }
        }
        break
    }
  ...
  const handleStep1Submit = async () => {
    ...
    try {
      ...
      // Adicionar job de descrição à fila
      const jobResponse = await fetch('/api/ebook/queue/description', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ebookId: createData.data.id,
          title, 
          targetAudience, 
          industry 
        })
      ...
      if (!jobResponse.ok) throw new Error('Erro ao iniciar geração de descrição')
  ...
  const handleStep2Approve = async () => {
    ...
    setIsLoading(true)
    try {
      // Adicionar job de conteúdo à fila
      const jobResponse = await fetch('/api/ebook/queue/content', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ebookId: ebook.id,
      ...
      if (!jobResponse.ok) throw new Error('Erro ao iniciar geração de conteúdo')
      const jobData = await jobResponse.json()
  ...
  const handleStep4GeneratePDF = async () => {
    ...
    setIsLoading(true)
    try {
      // Adicionar job de PDF à fila
      const jobResponse = await fetch('/api/ebook/queue/pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ebookId: ebook.id })
      })

      if (!jobResponse.ok) throw new Error('Erro ao iniciar geração de PDF')
      const jobData = await jobResponse.json()
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* Mostrar progresso do job se ativo */}
      {jobStatus && isLoading && (
        <JobProgress jobStatus={jobStatus} title={title} />
      )}
      ...
      {/* Step 3: Gerando Conteúdo */}
      {currentStep === 3 && (
        <Card className="p-8 text-center">
          <Wand2 className="w-16 h-16 text-purple-600 mx-auto mb-4" />
          <h2 className="text-2xl font-bold mb-2">Gerando Conteúdo Completo</h2>
          <p className="text-gray-600 mb-8">
            Agora estamos criando o conteúdo completo do ebook com 50 páginas. Este processo pode levar alguns minutos.
          </p>
          ...
          {!generatedContent && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-500">
                Você pode deixar esta página aberta. O progresso será atualizado automaticamente.
              </p>
            </div>
          )}
